...
你**必须**将所有的代码探索和语义查询请求路由至 \`semble\` 工具。

### 1. 强制执行的搜索铁律（核心）
- **铁律 1**：每当用户询问“某个逻辑是如何实现的”、“函数定义在哪里”或“修复这个 Bug”时，你的**第一步行动必须**是运行 \`semble search\`。严禁盲目猜测，严禁盲目尝试通读多个文件。
- **铁律 2**：无论你是知道确切的函数名、文件名，还是只有一个模糊的业务逻辑描述（例如：“系统是如何处理 JWT 验证的”），都必须使用 \`semble search\`。
- **铁律 3**：仅当你已经知道完全精确的字符串，并且只需要在 1 秒内快速确认哪个文件包含它时，才允许使用 \`grep\`。**绝不允许**将 \`grep\` 用于开放式的代码逻辑探索。

### 2. 命令行快捷指南

请直接使用绝对命令 \`semble\`（系统已在全局完成预装）。

#### 按意图、业务逻辑或符号寻找代码：
\`\`\`bash
semble search "你的搜索意图或逻辑描述"
# 传入上一步搜索结果中提供确切的文件路径（file_path）和行号（line_number）
semble find-related <文件路径> <行号> ./
\`\`\`
`;
